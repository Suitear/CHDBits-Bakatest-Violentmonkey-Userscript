...
// @include      http*://pterclub.net/*
// @include      http*://www.pterclub.net/*
// @grant        unsafeWindow
// @grant        GM_getValue
// @grant        GM_setValue
...
// --- PTerClub (猫站) 专用签到逻辑修正版 ---
      if (matchURL("pterclub.net")) {
        checkSignDate("PTerClub").then(() => {
        // 1. 使用原生选择器更稳健，精准定位 ID 为 do-attendance 的按钮
        const signBtn = document.getElementById('do-attendance');

        // 2. 只有当按钮存在，且文字包含“签到”时才执行（防止已经是“已签到”状态还去点）
        if (signBtn && signBtn.innerText.includes("签到")) {
            console.log("检测到猫站签到按钮，准备执行 Ajax 签到...");
            ...
            const closeTimer = setInterval(() => {
                detectCount++;
                // 猫站使用 jquery-confirm 插件，检测弹窗容器
                const jcBox = document.querySelector('.jconfirm-box');

                if (jcBox) {
                    const boxText = jcBox.innerText;
                    // 如果提示“签到成功”或“已经签到”
                    if (boxText.includes("成功") || boxText.includes("已经")) {
                        console.log("猫站签到反馈：" + boxText);

                        // 寻找并点击弹窗里的“确定”或“关闭”按钮（通常是 btn-default 或 btn-blue）
                        const okBtn = jcBox.querySelector('button.btn-default, button.btn-blue, button');
                        if (okBtn) okBtn.click();

                        // 记录签到状态，避免同日重复执行
                        setData("PTerClub", new Date().toLocaleDateString());
                        clearInterval(closeTimer);
                    }
                }

                // 如果检测 10 秒（10次）还没反应，强制停止，防止卡死
                if (detectCount > 10) clearInterval(closeTimer);
            }, 1000);
        } else {
            // 如果按钮文字已经是“已签到”或“明天再来”，则标记今日已完成
            if (signBtn && (signBtn.innerText.includes("已") || signBtn.innerText.includes("后"))) {
                 setData("PTerClub", new Date().toLocaleDateString());
            }
        }
    });
